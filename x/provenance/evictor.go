...
// exception to the rule that a derivative pins its source (ADR-113).
//
...
// sees nothing new.
//
// It works through the ordinary data plane of the store it was given, so both
// the extension's guard and any authorization gate wrapped around that store
// apply. Reaching for eviction is deliberate by construction: the caller has to
// build an Evictor and fill in a receipt, and a receipt without a stated reason
// and decider is refused.
type Evictor struct {
...
// SAME store handle the application writes through — the wrapped one. Passing an
// unwrapped store would skip the guard and any rights gate, which is exactly the
// mistake this type exists to make hard. The configuration comes from the index.
func NewEvictor(s store.DataStore, idx *Index) (*Evictor, error) {
	if s == nil || idx == nil {
		return nil, fmt.Errorf("provenance: evictor needs a store and an index")
	}
	return &Evictor{store: s, idx: idx}, nil
}
...
func (e *Evictor) Evict(ctx context.Context, id domain.ArtifactID, spec ReceiptSpec) error {
	...
	if _, err := e.store.Put(ctx, domain.Artifact{Payload: bytes.NewReader(doc)},
		WithProduction(Production{
			Inputs: []Input{{Ref: domain.HandleRef(id), Rel: e.idx.evictRel()}},
			Op:     EvictOp,
...
// disappearance. It is what a traversal that hit an unresolvable handle asks: the
// answer distinguishes a deliberate decision from data loss.
